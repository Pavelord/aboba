...
public class AudioRouteService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null) {
            ...
            if (targetDeviceName == null || targetDeviceName.isBlank()) targetDeviceName = "USB-гарнитура";
        }
    ...
    private void requestFocus() {
        ...
        focusRequest = new AudioFocusRequest.Builder(AudioManager.AUDIOFOCUS_GAIN_TRANSIENT_MAY_DUCK)
                ...
                .setOnAudioFocusChangeListener(change -> {
                    if (change == AudioManager.AUDIOFOCUS_GAIN) tryApplyRoute();
                })
                .build();
    ...
    private void tryApplyRoute() {
        ...
        if (target == null) {
            updateNotification("USB-устройство отсутствует в списке связи");
            return;
        }
        ...
        if (current == null || current.getId() != target.getId()) {
            ...
            updateNotification(ok ? "Маршрут активен: " + target.getProductName()
                    : "Android отказал: " + target.getProductName());
        }
    }

    private AudioDeviceInfo findTarget(List<AudioDeviceInfo> list) {
        for (AudioDeviceInfo device : list) if (device.getId() == targetDeviceId) return device;
        for (AudioDeviceInfo device : list) if (device.getType() == targetDeviceType) return device;
        for (AudioDeviceInfo device : list) {
            int type = device.getType();
            if (type == AudioDeviceInfo.TYPE_USB_HEADSET || type == AudioDeviceInfo.TYPE_USB_DEVICE
                    || type == AudioDeviceInfo.TYPE_USB_ACCESSORY) return device;
        }
        return null;
    }
    ...
    private void createChannel() {
        NotificationChannel channel = new NotificationChannel(CHANNEL_ID, "Удержание USB-аудио",
