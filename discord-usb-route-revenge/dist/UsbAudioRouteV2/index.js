var plugin = (() => {
  ...
  function safeToast(message) {
    ...
    } catch (error) { log("toast failed", error); }
  }
  function getReactNative() {
    try { return vendetta?.metro?.common?.ReactNative ?? null; }
    catch (error) { log("ReactNative lookup failed", error); return null; }
  }
  function findAudioModule() {
    ...
    for (const name of names) {
      ...
      } catch (error) { log("module lookup failed", name, error); }
    }
    return null;
  }
  ...
  async function readDevices() {
    ...
    } catch (error) { log("getAudioDevices failed", error); return []; }
  }
  ...
  async function getActiveDevice() {
    if (!audioModule || typeof audioModule.getActiveAudioDevice !== "function") return void 0;
    try { return await Promise.resolve(audioModule.getActiveAudioDevice()); }
    catch (error) { log("getActiveAudioDevice failed", error); return void 0; }
  }
  async function applyRoute(reason) {
    ...
    } catch (error) { log("setActiveAudioDevice failed", reason, error); }
  }
  function installPatch() {
    if (unpatch || !audioModule) return;
    try {
      const instead = vendetta?.patcher?.instead;
      if (typeof instead !== "function") { log("patcher unavailable; polling remains active"); return; }
      unpatch = instead("setActiveAudioDevice", audioModule, (args, original) => {
        try {
        ...
        } catch (error) { log("route interception failed", error); }
        return original(...args ?? []);
      });
      log("setActiveAudioDevice patched");
    } catch (error) { unpatch = void 0; log("patch installation failed; polling remains active", error); }
  }
  async function tick() {
  ...
  function onLoad() {
    try {
      ...
      timer = setInterval(() => { void tick(); }, 1e3);
    } catch (error) { log("onLoad guarded failure", error); }
  }
  function onUnload() {
    stopped = true;
    try {
